...
The `lung_source/` tree can be any depth so long as every image sits
in a leaf folder named covid, healthy, or other — e.g. the unprocessed
distribution's own `train/test/validation` split structure works as-is
(symlink your `dataset/unprocessed/` there). Only filenames are read;
pixels are not touched. The rename mapping
`lung_{cls}_{i}.png → original_filename` is reconstructed
deterministically from `sorted(filename_set_per_class)`, which is
seed-free — the `random.seed(42)` used by the original preprocessing
only affected template choice, not filenames.

No raw files are needed for BUSI, Thyroid, or Liver: their metadata is
already in the preprocessed captions.csv (BUSI, Thyroid) or in the
image filename prefix (Liver).
...
Usage (on the server, after `pip install vllm openpyxl`):
    ...
    CUDA_VISIBLE_DEVICES=3 python generate_llm_captions.py \
        --config config.json \
        --raw-metadata-root /home/zhuoyangl/tryout/bmi702/data/metadata \
...
_LUNG_EXCEL_NAME = "lung_metadata.xlsx"
_LUNG_SOURCE_DIRNAME = "lung_source"
_LUNG_CLASSES = ("covid", "healthy", "other")
_LUNG_IMG_EXTS = {".jpg", ".jpeg", ".png"}
_LUNG_CONCLUSION_COL = 202
_LUNG_IMG_COLS = [
...
def load_lung(captions_csv: Path, raw_root: Path) -> dict[str, dict[str, str]]:
    import openpyxl  # lazy import — only required for lung

    excel = raw_root / _LUNG_EXCEL_NAME
    if not excel.is_file():
        raise FileNotFoundError(
            f"[Lung] metadata Excel not found: {excel}. "
            f"Save the lung dataset's metadata.xlsx at that path (you can "
            f"rename it from metadata.xlsx to lung_metadata.xlsx)."
        )
    source_root = raw_root / _LUNG_SOURCE_DIRNAME
    if not source_root.is_dir():
        raise FileNotFoundError(
            f"[Lung] source image tree not found: {source_root}. "
            f"Symlink your unprocessed lung dataset there. Any depth works "
            f"as long as every image's parent directory is named one of "
            f"{list(_LUNG_CLASSES)} — e.g. "
            f"{source_root}/{{train,test,validation}}/{{covid,healthy,other}}/ "
            f"or {source_root}/{{covid,healthy,other}}/. Only filenames are "
            f"read (pixels are ignored); they must be the exact set that "
            f"rename_and_combine_lung.py originally saw."
        )

    # Rebuild the old→new filename mapping that rename_and_combine_lung.py
    # produced: per class, sort all filenames alphabetically and enumerate
    # from 1 as lung_{cls}_{i}.png. The rename is deterministic and
    # seed-free, so we just need the filename SET per class — we recover
    # that by unioning every image under `lung_source/` whose parent dir
    # name is `cls`. This replicates combine_lung_images.py (which pools
    # the three data splits into one folder per class) at the filename
    # level only.
    filenames_by_cls: dict[str, set[str]] = {c: set() for c in _LUNG_CLASSES}
    for p in source_root.rglob("*"):
        if not p.is_file():
            continue
        if p.suffix.lower() not in _LUNG_IMG_EXTS:
            continue
        cls = p.parent.name.lower()
        if cls in filenames_by_cls:
            filenames_by_cls[cls].add(p.name)

    old_to_new: dict[str, str] = {}
    for cls in _LUNG_CLASSES:
        files = sorted(filenames_by_cls[cls])
        if not files:
            raise FileNotFoundError(
                f"[Lung] no image files found under {source_root} whose "
                f"parent directory is named {cls!r}. Check that your source "
                f"tree has a `{cls}` leaf folder."
            )
        for i, fname in enumerate(files, start=1):
            old_to_new[fname] = f"lung_{cls}_{i}.png"

    # Load Excel and re-key by the NEW (renamed) filename.
    wb = openpyxl.load_workbook(excel, data_only=True, read_only=True)
    ws = wb.active
    if ws is None:
        raise ValueError(f"[Lung] {excel}: no active sheet")

    out: dict[str, dict[str, str]] = {}
    for raw_row in ws.iter_rows(min_row=2, values_only=True):
        row = list(raw_row) if raw_row else []
        if len(row) <= _LUNG_CONCLUSION_COL:
            continue

        patient_fields: dict[str, str] = {}
        age = clean_value(row[4])
        if age is not None:
            try:
                patient_fields["Age"] = str(int(float(age)))
            except (TypeError, ValueError):
                patient_fields["Age"] = age
        gender = clean_value(row[5])
        if gender is not None:
            patient_fields["Gender"] = gender
        preg = clean_value(row[6])
        if preg is not None and preg.lower() in {"yes", "y", "true", "1"}:
            patient_fields["Pregnant"] = "yes"
        concl = clean_value(row[_LUNG_CONCLUSION_COL])
        if concl is not None:
            patient_fields["Conclusion"] = concl

        for col_idx, region_label in _LUNG_IMG_COLS:
            if col_idx >= len(row):
                continue
            fname = row[col_idx]
            if not fname:
            ...
            if not fname:
                continue
            new_name = old_to_new.get(fname)
            if new_name is None:
                # This row's filename wasn't in lung_resized/ — must mean
                # the Excel lists images that weren't part of the curated
                # training set. Skip silently; we'll verify completeness
                # against captions.csv below.
                continue
            fields = dict(patient_fields)
            fields["Tissue"] = "lung"
            fields["View"] = region_label
            findings = clean_value(
                row[col_idx + 1] if col_idx + 1 < len(row) else None
            )
            if findings is not None:
                fields["Findings"] = findings
            out[new_name] = fields

    # Every basename in captions.csv MUST have metadata attached.
    missing = [bn for bn in _basenames_in_csv(captions_csv, "Lung")
               if bn not in out]
    if missing:
        raise KeyError(
            f"[Lung] {len(missing)} basenames in {captions_csv} have no "
            f"metadata after rebuilding the rename map (e.g. {missing[:3]}). "
            f"The filename set under {source_root} differs from what "
            f"rename_and_combine_lung.py originally saw — check for extra "
            f"or missing images under {{covid,healthy,other}}/."
        )
    return out
...
DATASET_LOADERS: list[tuple[str, Callable[[Path, Path], dict], str]] = [
    ...
    ("Lung_ultrasound",  load_lung,     "Lung (metadata.xlsx + lung_resized/)"),
...
def main():
    ...
    ap.add_argument("--raw-metadata-root", required=True,
                    help="Directory containing BrEaST-Lesions-USG-clinical-"
                         "data-Dec-15-2023.xlsx, lung_metadata.xlsx, and "
                         "lung_source/ (any tree whose leaf folders are "
                         "named covid/healthy/other — filenames only, "
                         "pixels ignored).")
    ...
    combined = pd.concat(
        [df[["image_path", "dataset"]].assign(__split=sp)
